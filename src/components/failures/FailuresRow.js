  ...
  return (
    ...
    >
     ...
     <div className="flex sm:hidden pl-4 mb-4 rounded-md border-b border-[#001c38] bg-[#022a52] cursor-pointer flex flex-col"
      style={{ borderLeft: `8px solid ${getStatusColor(data.gravity?.toLowerCase())}` }}
    >
      <div onClick={handleRowClick} className="pt-3 flex flex-col justify-center min-h-[60px]" style={{ height: "-webkit-fill-available" }}>

        <div className="mb-2" style={{ height: "-webkit-fill-available" }}>
          <StatusBadge date={data.date} gravity={data.gravity} />
        </div>
        <p className="text-white text-[18px] font-semibold truncate">{data.title}</p>
        <p className="text-white/60 text-[16px] text-sm truncate">
      ...
      <div className="flex items-center">
        <div className="pb-3 pt-3 pr-3 flex items-center justify-center cursor-pointer" onClick={() => setIsOpen(true)} style={{ height: "-webkit-fill-available" }}>
          <div className="flex gap-4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 576 512"
        ...
        <div className="p-3 flex items-center justify-center" style={{ height: "-webkit-fill-available" }}>
          {data.userExecution}
        </div>    
      </div>
