...
export default function NoteModal({ onClose, id }) {
  ...
  const { user, addNote, getNotes } = useUser();
  ...
  useEffect(() => {
    ...
    if (existing) {
      if (existing.text) setTextNotes(existing.text);
      if (existing.photo) setPhotoFiles(existing.photo);
      if (existing.vocal) setAudioFiles(existing.vocal);
    }
  }, [id]);

  const saveNoteToContext = (type, content) => {
    addNote(id, type, content); // append to context
  };

  const handleTextAdd = () => {
    if (textInput.trim()) {
      const updated = [...textNotes, textInput.trim()];
      setTextNotes(updated);
      saveNoteToContext("text", textInput.trim());
      setTextInput("");
    }
  };
  ...
  const handlePhotoAdd = (e) => {
    ...
    if (file) {
      const updated = [...photoFiles, file];
      setPhotoFiles(updated);
      saveNoteToContext("photo", file);
    }
  };
  ...
  const handleAudioAdd = (e) => {
    const file = e.target.files[0];
    if (file) {
      const updated = [...audioFiles, file];
      setAudioFiles(updated);
      saveNoteToContext("vocal", file);
    }
  };
