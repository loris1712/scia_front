...
const SpareRow = ({ data, onRemove }) => {
  ...
  return (
    <div>
      ...
      <div className="flex sm:hidden mb-4 rounded-md bg-[#022a52] cursor-pointer flex flex-col p-4">
        <div onClick={handleRowClick} className="flex flex-col justify-center"> 
          <p className="text-white text-[18px] font-semibold truncate">{data?.Spare?.Part_name ?? "Nome non disponibile"}</p>
        </div>
        <div className="text-white gap-2 mt-2" style={{ height: "-webkit-fill-available" }}>
          <p className="text-[18px] text-white">{data?.Spare?.Serial_number ?? "Nome non disponibile"}</p>
        </div>
        <div className="cursor-pointer" onClick={() => setIsOpen(true)} style={{ height: "-webkit-fill-available" }}>
          <div className="flex gap-4">
          {data?.Spare?.company ?? "Nome non disponibile"}
          </div>
        </div>

        <div className="flex items-center w-[fit-content] ml-auto mr-auto">
          <div
